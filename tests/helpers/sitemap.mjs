...
export async function readSitemapUrls() {
  ...
  assert.equal(children.length, 8, "sete frentes e grupo");
  assert.equal(new Set(children.map(url => url.href)).size, 8);
  const urls = [];
  for (const child of children) {
    assert.equal(child.origin, "https://newtractor.com.br");
    assert.match(child.pathname, /^\/sitemap-(rodantes|hidrautractor|usinagem|calderaria|parts|services|techtractor|grupo)\.xml$/);
