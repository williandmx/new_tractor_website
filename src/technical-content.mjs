  ...
  const definitions = [
    {
      ...
      content: section("escolher-guia", "Comece pela decisão que sua equipe precisa tomar", list([
        ...
        `<a href="${guideLinks[3][1]}">${guideLinks[3][0]}</a>: identificação, sintomas e aplicação para preparar a avaliação do cilindro.`,
    ...
    },
    {
      ...
      ])) + section("sinais-cilindro", "Registre o sintoma sem fechar um diagnóstico", `<p>Descreva vazamento percebido, risco ou corrosão na haste, movimento lento, perda de força, folga, ruído ou dificuldade para manter a posição. Informe se o sinal aparece a frio ou a quente, com carga ou sem carga, em qual movimento e desde quando.</p><p>Fotos contextualizam a consulta; a inspeção e os testes apropriados completam a avaliação. Informe o histórico de troca, intervenção, fluido e condição de uso, separando dados disponíveis de itens pendentes de conferência.</p><p>Na consulta, “reparo” pode significar um kit de reparação ou vedação, uma peça isolada ou uma intervenção no conjunto. Descreva qual escopo está sendo considerado para encaminhar a demanda.</p>`) + section("aplicacao-cilindro", "A aplicação muda o contexto da avaliação", `<p>Na mineração, relacione o cilindro à escavadeira, trator ou perfuratriz e à frente de trabalho. No setor florestal, informe se está em equipamento de manejo, carregamento ou apoio. No agronegócio, diferencie trator, colheitadeira e implemento. Em construção e terraplenagem, registre obra, tarefa e acesso. Em locadoras, associe o componente ao patrimônio e ao momento de devolução ou manutenção.</p><p>Use esses contextos para registrar a operação, a prioridade e a compatibilidade a avaliar.</p>`) + section("cilindros-por-maquina", "Cilindros de escavadeira, trator de esteira ou carregadeira?", `<p>Na <a href="/equipamentos/escavadeiras/">escavadeira, separe os cilindros da lança, do braço e da caçamba</a>. No <a href="/equipamentos/tratores-de-esteira/">trator de esteira, identifique os movimentos da lâmina e os acessórios</a>. Na <a href="/equipamentos/pas-carregadeiras/">pá carregadeira, diferencie elevação, inclinação e direção</a>. A configuração e o código precisam acompanhar o nome do componente.</p><p>O termo “pistão hidráulico” também aparece na conversa comercial. Diga se você se refere ao cilindro completo ou ao êmbolo interno. Um pedido de “reparo de pistão” deve esclarecer se envolve uma peça, um kit ou a avaliação do conjunto.</p>`) + section("peca-ou-conjunto", "Peça isolada ou cilindro completo?", `<p>Esclareça se a consulta é sobre o cilindro completo ou sobre haste, tubo, tampa, êmbolo (pistão), olhais, conexões e elementos de vedação. O êmbolo ou pistão é uma parte do conjunto; código, desenho, amostra ou fotos devem deixar claro se a demanda é pela peça ou pelo cilindro completo.</p><p>Quando a plaqueta ou o catálogo informar o tipo, registre se o cilindro é de simples ação ou de dupla ação. Essa classificação descreve a forma de atuação do cilindro e ajuda a diferenciar aplicações; a equipe ainda precisa conferir o componente e o escopo.</p><p>A HidrauTractor executa <a href="/hidrautractor/recuperacao-cilindros-hidraulicos/">recuperação de cilindros hidráulicos</a> e <a href="/hidrautractor/fabricacao-cilindros-hidraulicos/">fabricação de novos conjuntos</a>. A identificação e a aplicação orientam o plano de reparo ou o projeto. Para outras peças, consulte <a href="/parts/">Parts</a>; demandas de geometria e estrutura podem envolver <a href="/usinagem/">Usinagem</a> ou <a href="/calderaria/">Calderaria</a>.</p>`) + section("perguntas-recuperacao", "Perguntas para avaliar uma recuperação", list([
    ...
    },
    {
      ...
      description: "Saiba o que enviar para cotar a fabricação de peças de máquinas pesadas: código, desenho, amostra, modelo, aplicação e quantidade.",
      ...
      ])) + section("nova-ou-recuperacao", "Peça nova ou recuperação da peça existente?", `<p>Se a necessidade é uma peça nova, encaminhe a identificação e os requisitos para <a href="/parts/">New Tractor Parts</a>. A Parts fabrica peças sob demanda, com especificação e prazo definidos para a aplicação da máquina.</p><p>Se existe uma peça desgastada a recuperar, apresente a condição para <a href="/servicos/usinagem-componentes-maquinas-pesadas/">Usinagem</a>, <a href="/calderaria/">Calderaria</a> ou <a href="/rodantes/">Rodantes</a>, conforme o componente. Recuperar uma geometria e fabricar uma peça nova são escopos distintos.</p>`) + section("sem-codigo", "O que enviar quando não há código?", `<p>Envie fotos do conjunto e da peça, sua posição na máquina, modelo do equipamento e medidas disponíveis identificadas como preliminares. Informe se existe amostra ou desenho. Use a fotografia como referência inicial e complemente-a com a documentação necessária para avaliar material, tolerância e compatibilidade.</p><p>Na comparação de propostas, verifique se quantidade, condição da peça, documentação, transporte e escopo são equivalentes. Informe a data em que a peça será necessária para combinar o prazo de fabricação e entrega.</p>`),
    },
    {
      ...
      ])) + section("recuperacao-integrada", "Quando a demanda envolve outros processos", `<p>Se o componente apresenta trincas, deformação ou demanda de solda, a avaliação pode envolver a <a href="/calderaria/">Calderaria</a>. Para rosários, sapatas, roletes, rodas-guia e trucks, o caminho começa pela <a href="/servicos/manutencao-material-rodante/">manutenção de material rodante</a>.</p><p>Para uma peça nova, a <a href="/parts/">New Tractor Parts</a> executa fabricação sob demanda a partir dos requisitos da aplicação. O <a href="/guias/cotacao-pecas-maquinas-pesadas/">guia de cotação de peças</a> ajuda a organizar código, desenho e requisitos.</p>`) + section("base-usinagem", "Enviar a peça e combinar a logística", `<p>Para operações fora de Belo Horizonte, envio, coleta, mobilização e prazo são avaliados conforme a cidade, a peça e a prioridade. Informe a cidade desde o primeiro contato.</p><p>Conheça a <a href="/usinagem/">New Tractor Usinagem</a> e envie os dados da peça para avaliar o próximo passo.</p>`),
    },
  ];
  return definitions.map((entry) => {
    const revised = ["/guias/", "/guias/avaliacao-maquinas-linha-amarela/", "/guias/avaliacao-cacambas-conchas/", "/guias/inspecao-material-rodante/", "/guias/cotacao-pecas-maquinas-pesadas/", "/servicos/usinagem-componentes-maquinas-pesadas/"].includes(entry.route);
    const lastModified = revised ? "2026-09-10" : entry.lastModified ?? (entry.service ? "2026-09-07" : "2026-09-08");
    ...
    return {
      ...
      active: entry.service ? "servicos" : "guias", lastModified,
      ...
      body: pageHero({ eyebrow: entry.service ? "USINAGEM" : "GUIAS PARA MANUTENÇÃO", title: entry.heading, intro: entry.intro,
        ...
        imageWidths: entry.widths, imageWidth: entry.width, imageHeight: entry.height }) + (!entry.service && entry.route !== "/guias/" ? `<div class="shell article-credit"><p>Conteúdo editorial do Grupo New Tractor · Atualizado em ${new Date(`${lastModified}T12:00:00Z`).toLocaleDateString("pt-BR", { dateStyle: "long", timeZone: "UTC" })}</p></div>` : "") + entry.content + contactBand("Solicite apoio para a manutenção da sua máquina"),
    };
  });
}
